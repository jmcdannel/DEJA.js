...
The Monitor app is hosted at [monitor.dejajs.com](https://monitor.dejajs.com). Open it in any browser, log in with your DEJA Cloud account, and select your layout to begin monitoring.

### Developer Mode

If you're working with the full monorepo, you can also run the Monitor locally in development mode:

```bash
pnpm --filter=deja-monitor dev
```

The Monitor app requires authentication (`requireAuth`) and a selected layout (`requireLayout`) before the dashboard loads.
