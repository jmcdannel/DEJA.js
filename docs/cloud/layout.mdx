...
The Layout section is where you manage the hardware infrastructure behind your model railroad. This includes registering hardware devices (CommandStations, Arduino Mega boards, ESP32 boards over USB or WiFi, and Raspberry Pi Pico W controllers), viewing available serial ports, reviewing loaded firmware modules, and organizing your layout with tags. Layout data is stored in Firebase Firestore at `layouts/{layoutId}`.
...
- **Device Type** -- The type of hardware. Supported device types include DCC-EX CommandStation (`dcc-ex`), DEJA Arduino Mega (`deja-arduino`), ESP32 over USB (`deja-esp32`), ESP32 over WiFi/MQTT (`deja-esp32-wifi`), Raspberry Pi Pico W (`deja-mqtt`), and others defined in the `deviceTypes` constant.
...
For DCC-EX, DEJA Arduino, and ESP32 devices (`deja-arduino`, `deja-esp32`, `deja-esp32-wifi`), an expandable "Developer Configuration" panel generates a `config.h` code snippet that can be used in the Arduino/ESP32 firmware. For `deja-esp32-wifi` it also bakes in `WIFI_SSID`, `WIFI_PASSWORD`, `MQTT_BROKER`, and the rest of the MQTT credentials. The panel also surfaces the exact PlatformIO and Arduino CLI build/upload commands for the device's FQBN, pulled from `getCliDeployCommands()` so they always match what `deja deploy` runs under the hood. This auto-generated configuration includes:
...
This section is designed to help firmware developers quickly generate the configuration code needed when flashing new Arduino, ESP32, or Pico W boards. See [Arduino & ESP32 Setup](/docs/io/arduino-setup) and [Pico W Setup](/docs/io/pico-w-setup) for the full firmware setup walkthroughs.
...
| `type` | string | Device type (`dcc-ex`, `deja-arduino`, `deja-esp32`, `deja-esp32-wifi`, `deja-mqtt`, etc.) |
