...
![DCC-EX Console](/screenshots/cloud_desktop_dashboard.png)
...
![DCC-EX command log with LCD panels](/screenshots/cloud_desktop_dashboard.png)

## How Commands Flow

When you send a command from the console:

1. The Cloud app sends the command to Firebase.
2. The DEJA Server picks up the command and forwards it over USB serial to the CommandStation.
3. The CommandStation processes the command and sends any response back.
4. The server broadcasts the response to all connected apps.
5. The DCC Log displays the response in the console.
...
- [Layout Configuration](/docs/cloud/layout) -- Configure the devices and serial ports used by the console.
- [Locomotive Roster](/docs/cloud/roster) -- Manage locomotives at a higher level of abstraction.
