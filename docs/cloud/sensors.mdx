...
Physical sensor (IR, current, reed, etc.)
       |
       v
Hardware device (Arduino, ESP32, or Pico W)
       |
       v
...
Sensor pins are defined in the device firmware configuration. For Arduino and ESP32 devices (`deja-arduino`, `deja-esp32`, `deja-esp32-wifi`), sensor pins are configured in the `config.h` file:
...
For Pico W devices (`deja-mqtt`) running CircuitPython, sensor pins are configured in the device's `config.json` file (or in a layout-specific override under `io/layouts/{layoutId}/deja-pico-w/{deviceId}/config.json`).
