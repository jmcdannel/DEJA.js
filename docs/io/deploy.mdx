...
pnpm --filter=@deja/io build -- --layout <layoutId> --device <deviceId>
...
> 🛜 If you ran `pnpm build` non-interactively, the WiFi/MQTT credentials in `deja-esp32-wifi/config.h` will be empty — edit them by hand before flashing or re-run the interactive wizard.
