...
> 🛜 Before flashing `deja-esp32-wifi` by hand: confirm `WIFI_SSID`, `WIFI_PASSWORD`, `MQTT_BROKER`, `DEVICE_ID`, `LAYOUT_ID`, and `TOPIC_ID` are set correctly in `deja-esp32-wifi/config.h`. The deploy wizard bakes these in automatically; non-interactive `pnpm build:firmware` runs leave them empty.
