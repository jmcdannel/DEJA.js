---
title: Architecture
description: System architecture of DEJA.js — monorepo structure, communication layers, data flow, and the technology stack.
section: docs
order: 4
---
...
```
DEJA.js/
├── apps/           Frontend applications and the backend server
...
| `apps/cloud` | Vue 3 layout management and configuration hub |
| `apps/monitor` | Vue 3 diagnostics and logging dashboard |
| `apps/tour` | Vue 3 interactive tour and effects runner |
| `apps/server` | Node.js backend that bridges browsers to DCC-EX hardware |
...
| Package | Purpose |
|---------|---------|
| `@repo/modules` | Core business logic -- composables and types for locos, turnouts, effects, signals, routes, and layouts |
| `@repo/ui` | Shared Vue component library (LocoAvatar, TurnoutSwitch, EmergencyStop, TrackPower, and more) |
...
| `@repo/firebase-config` | Firebase client SDK and Admin SDK initialization |
...
| `@repo/utils` | Common utility functions (slugify, helpers) |
...
dejaCommands/{layoutId}         -- System command queue (device connect, port list, status)
```
...
Messages follow a `{ action, payload }` JSON format. See the [WebSocket Protocol](/docs/dev/websocket-protocol) documentation for the full specification.
...
```
  User taps speed slider in Throttle app (browser)
       |
       v
  Vue component calls useDcc().sendSpeed({ address, speed })
       |
       v
  ...
  Firebase RTDB triggers child_added event
       |
       v
  Server: dejaCloud.ts listener fires
...
│   ├── deja.ts         System command handler (non-DCC)
...
| Technology | Purpose |
|------------|---------|
| **Vue 3** | UI framework (Composition API with `<script setup lang="ts">`) |
...
- **Vite** -- Dev server and production bundler for all Vue and Next.js apps.
