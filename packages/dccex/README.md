...
Provides the composable for sending DCC-EX commands from frontend apps to the server via Firebase RTDB. This package is the bridge between the Vue UI layer and the DCC-EX CommandStation hardware.

> 📐 See [ARCHITECTURE.md](../../ARCHITECTURE.md) for the full data flow from browser to track.
...
|----------|-----------|-------------|
| `setFunction` | `(address: number, func: number, state: boolean) => Promise<void>` | 🎛️ Toggle a locomotive function (headlight, horn, bell, etc.) |
...
- **☁️ Cloud mode** (default) -- Writes commands to Firebase RTDB.
...
| Function | `<F addr func state>` | `dcc.sendFunction({ address, func, state })` | 🎛️ Toggle a locomotive function |
...
|-----------|-------|-------------|
| `addr` | 1--9999 | 🏷️ DCC decoder address |
| `speed` | 0--126 | 🚀 Absolute speed (0 = stop, 126 = maximum) |
| `dir` | 0 or 1 | ➡️ Direction: `1` = forward, `0` = reverse |
...
|-----------|-------|-------------|
| `addr` | 1--9999 | 🏷️ DCC decoder address |
| `func` | 0--28 | 🎛️ Function number (F0 = headlight, F1 = bell, F2 = horn, etc.) |
...
1. ✔️ The command must be a string.
2. ✔️ The command must not exceed 200 characters.
3. ✔️ The command must not contain angle brackets (`<`, `>`) or newline characters (`\n`, `\r`) -- these would break the DCC-EX framing protocol.
