...
async function handleFacebookSignup() {
  ...
  error.value = null
  try {
    const cred = await signInWithPopup(auth, facebookAuthProvider)
    emit('signup', cred.user)
  } catch (err: unknown) {
    const firebaseErr = err as { message?: string }
    error.value = firebaseErr.message ?? 'Facebook signup failed'
  } finally {
...
async function handleAppleSignup() {
  ...
  error.value = null
  try {
    const cred = await signInWithPopup(auth, appleAuthProvider)
    emit('signup', cred.user)
  } catch (err: unknown) {
    const firebaseErr = err as { message?: string }
    error.value = firebaseErr.message ?? 'Apple signup failed'
  } finally {
...
async function handleMicrosoftSignup() {
  ...
  error.value = null
  try {
    const cred = await signInWithPopup(auth, microsoftAuthProvider)
    emit('signup', cred.user)
  } catch (err: unknown) {
    const firebaseErr = err as { message?: string }
    error.value = firebaseErr.message ?? 'Microsoft signup failed'
  } finally {
  ...
  <div class="auth-page">
    <div class="auth-container">
      ...
      <div class="auth-card">
        ...
        <v-alert v-if="error" type="error" class="mb-4" closable density="compact" @click:close="error = null">
          {{ error }}
        </v-alert>

        <!-- Social Auth -->
        <div class="flex flex-col gap-3 mb-6">
          <button class="social-btn" :disabled="anyLoading" @click="handleGoogleSignup">
          ...
          <button class="social-btn" :disabled="anyLoading" @click="handleGithubSignup">
          ...
          <button class="social-btn" :disabled="anyLoading" @click="handleFacebookSignup">
          ...
          <button class="social-btn" :disabled="anyLoading" @click="handleAppleSignup">
          ...
          <button class="social-btn" :disabled="anyLoading" @click="handleMicrosoftSignup">
