...
<template>
  <v-sheet class="bg-gradient-to-r from-pink-500 to-indigo-800 flex flex-row gap-2 justify-between items-center p-2 rounded-2xl shadow-lg bg-gradient-border ">
    <slot name="title">
      <h2 class="text-xl text-purple-400 opacity-70 font-bold">Consist</h2>
    </slot>
    <ol v-if="loco?.consist?.length" class="flex flex-row items-center justify-between overflow-x-auto p-1">
      ...
      <li>
        <v-btn @click="settingsDialogOpen = true" icon="mdi-cog" />
      </li>
    </ol>
    <template v-else>
      <v-btn color="purple" @click="settingsDialogOpen = true" icon="mdi-cogs" size="36" variant="tonal" />
    </template>
  </v-sheet>
