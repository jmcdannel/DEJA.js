...
import { useTimeoutFn, useTransition } from '@vueuse/core'
...
// 🎛️ Animated rotation angle for the handle
const targetAngle = ref(state.value ? 0 : -55)
const angle = useTransition(targetAngle, {
  duration: 300,
  transition: [0.4, 0, 0.6, 1],
})
const handleTransform = computed(() => `rotate(${angle.value} 300.31 710.84)`)

// Watch state changes to update the target angle
watch(state, (newState) => {
  targetAngle.value = newState ? 0 : -55
})
