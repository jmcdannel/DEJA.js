...
If no turnouts are configured for the current layout, the screen displays an empty state with a swap icon and the message "No turnouts -- Add turnouts to control track switches." Turnouts are created and configured in the [Cloud app](/docs/apps/cloud).

## Turnouts in Other Views

Turnouts also appear in the [Conductor View](/docs/apps/throttle/conductor) (right pane) and on the interactive [Routes](/docs/apps/throttle/routes) track maps, where they can be toggled directly on the SVG map by tapping the turnout element.
