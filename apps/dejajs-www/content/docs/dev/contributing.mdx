---
title: Contributing to DEJA.js
description: Development setup, workflow, coding conventions, and guidelines for contributing to the DEJA.js model railroad control system.
section: dev
order: 9
---
...
|----------|-------------|
| `LAYOUT_ID` | Your layout identifier in Firebase |
...
pnpm --filter=deja-cloud dev       # Cloud app only
pnpm --filter=deja-serverts dev    # Server only
pnpm deja                          # Server + Monitor together
```
...
Do not bypass ESLint with `// eslint-disable` without a specific reason documented in the comment.
