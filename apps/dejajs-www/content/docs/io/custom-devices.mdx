...
> 💡 If you just want a working ESP32 WiFi build without writing your own firmware, use the stock **`deja-esp32-wifi`** target — see [Arduino & ESP32 Setup](/docs/io/arduino-setup). It uses exactly the topology and command format described on this page, so you can reuse the protocol details below to add custom actions on top of the stock sketch.
