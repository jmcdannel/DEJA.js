...
function signalColor(i: number) {
  ...
  if (i === activeStep.value) return '#38bdf8'
  return '#7c3aed'
}
function labelColor(i: number) {
  if (i < activeStep.value) return '#4ade80'
  if (i === activeStep.value) return '#e0f2fe'
  return '#a78bfa'
}
function labelWeight(i: number) {
  return i === activeStep.value ? 'bold' : '600'
}
...
// Train position
const trainX = computed(() => sx[activeStep.value])
const trainY = computed(() => sy[activeStep.value])

// Track progress 0-1
const progress = computed(() => activeStep.value / (steps.length - 1))
...
onMounted(() => {
  ...
  // Measure actual SVG path length
  if (trackRef.value) {
    pathLength.value = trackRef.value.getTotalLength()
  }
})
onUnmounted(() => clearInterval(phraseTimer))
...
<template>
  <v-container class="max-w-4xl mx-auto py-8">
    <h1 class="text-2xl font-bold text-sky-100 mb-2">🚂 DEJA Tracker</h1>
    <p class="text-sm opacity-60 mb-4">Click step buttons to move the train</p>

    <div class="flex gap-2 mb-8 flex-wrap">
      <v-btn
        v-for="(step, i) in steps"
        :key="i"
        :color="i === activeStep ? 'cyan' : 'default'"
        :variant="i === activeStep ? 'flat' : 'tonal'"
        size="small"
        @click="activeStep = i"
      >
        {{ i + 1 }}. {{ step }}
    ...
    <div class="tracker-card">
      <svg viewBox="0 0 630 190" class="w-full block" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <!-- Glows -->
