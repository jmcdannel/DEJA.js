...
export const broadcast = async (data: string | BroadcastMessage): Promise<void> => {
  try {
    if (ENABLE_MQTT) {
      await dejaMqtt.send(JSON.stringify(data))
    }
    if (ENABLE_WS) {
      await wsServer.send(JSON.parse(data.toString()))
    }
    // if (ENABLE_DEJACLOUD) {
