...
export async function handleEffect(payload: Effect): Promise<void> {
  ...
  const conn = payload.device ? layout.connections()?.[payload.device] : undefined
  if (!conn?.isConnected) {
  ...
  if (
    ...
    conn.send(conn.port, JSON.stringify([command]))
  } else if (layoutDevice?.connection === 'wifi' && conn.publish && conn.topic) {
    conn.publish(conn.topic, JSON.stringify(command), true)
  } else if (layoutDevice?.type === 'dcc-ex') {
    await dcc.sendCommand(JSON.stringify(command))
  }
}

export async function handleEffectChange(snapshot: DocumentData): Promise<void> {
