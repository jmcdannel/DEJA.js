<script setup lang="ts">

</script>
<template>
  <h1 class="flex items-center">
    <router-link to="/">
      <img src="/images/android-chrome-192x192.png" alt="Deja Throttle Logo" class="w-10 h-10" />
    </router-link>
    <router-link to="/" class="hidden sm:flex items-center normal-case text-xl">
      <span class="bg-clip-text bg-gradient-to-r from-sky-500 to-sky-800 font-extrabold">
        DEJA
      </span> 
      <span class="ml-1 text-sm bg-clip-text bg-gradient-to-r from-primary to-white uppercase">
        Throttle
      </span>
    </router-link>
  </h1>
</template>
