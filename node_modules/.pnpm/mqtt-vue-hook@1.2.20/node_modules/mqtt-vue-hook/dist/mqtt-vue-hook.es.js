...
const eq = (filter, topic, handleSharedSubscription = false) => {
    ...
    return function() {
      ...
      (function(process, global2) {
        (function() {
          ...
          const debug2 = require2("debug")("mqttjs:client");
            ...
            if (client.options.protocolVersion === 5) {
              if (packet.cmd === "publish") {
                ...
                if (client.topicAliasSend) {
                  if (alias) {
                    if (topic.length !== 0) {
                      debug2("applyTopicAlias :: register topic: %s - alias: %d", topic, alias);
                      if (!client.topicAliasSend.put(topic, alias)) {
                        debug2("applyTopicAlias :: error out of range. topic: %s - alias: %d", topic, alias);
                    ...
                    if (topic.length !== 0) {
                      if (client.options.autoAssignTopicAlias) {
                        ...
                        if (alias) {
                          ...
                          debug2("applyTopicAlias :: auto assign(use) topic: %s - alias: %d", topic, alias);
                        } else {
                          alias = client.topicAliasSend.getLruAlias();
                          client.topicAliasSend.put(topic, alias);
                          packet.properties = __spreadProps(__spreadValues({}, packet.properties), { topicAlias: alias });
                          debug2("applyTopicAlias :: auto assign topic: %s - alias: %d", topic, alias);
                        }
                        ...
                        if (alias) {
                          packet.topic = "";
                          packet.properties = __spreadProps(__spreadValues({}, packet.properties), { topicAlias: alias });
                          debug2("applyTopicAlias :: auto use topic: %s - alias: %d", topic, alias);
                        }
                      }
                    }
                  }
          ...
          function sendPacket(client, packet, cb) {
            debug2("sendPacket :: packet: %O", packet);
            debug2("sendPacket :: emitting `packetsend`");
            client.emit("packetsend", packet);
            debug2("sendPacket :: writing to stream");
            const result = mqttPacket.writeToStream(packet, client.stream, client.options);
            debug2("sendPacket :: writeToStream result %s", result);
            if (!result && cb && cb !== nop) {
              debug2("sendPacket :: handle events on `drain` once through callback.");
              client.stream.once("drain", cb);
            } else if (cb) {
              debug2("sendPacket :: invoking cb");
              cb();
            }
          }
          function flush(queue) {
            if (queue) {
              debug2("flush: queue exists? %b", !!queue);
          ...
          function flushVolatile(queue) {
            if (queue) {
              debug2("flushVolatile :: deleting volatile messages from the queue and setting their callbacks as error function");
              Object.keys(queue).forEach(function(messageId) {
          ...
          function storeAndSend(client, packet, cb, cbStorePut) {
            debug2("storeAndSend :: store packet with cmd %s to outgoingStore", packet.cmd);
          ...
          function MqttClient(streamBuilder, options) {
            ...
            debug2("MqttClient :: options.protocol", options.protocol);
            debug2("MqttClient :: options.protocolVersion", options.protocolVersion);
            debug2("MqttClient :: options.username", options.username);
            debug2("MqttClient :: options.keepalive", options.keepalive);
            debug2("MqttClient :: options.reconnectPeriod", options.reconnectPeriod);
            debug2("MqttClient :: options.rejectUnauthorized", options.rejectUnauthorized);
            debug2("MqttClient :: options.topicAliasMaximum", options.topicAliasMaximum);
            this.options.clientId = typeof options.clientId === "string" ? options.clientId : defaultId();
            debug2("MqttClient :: clientId", this.options.clientId);
            ...
            if (options.topicAliasMaximum > 0) {
              if (options.topicAliasMaximum > 65535) {
                debug2("MqttClient :: options.topicAliasMaximum is out of range");
              } else {
            ...
            this.on("connect", function() {
              ...
              function deliver() {
                ...
                packet = entry.packet;
                debug2("deliver :: call _sendPacket for %o", packet);
                let send = true;
                ...
                if (send) {
                  ...
                  debug2("messageId: %d has already used. The message is skipped and removed.", packet.messageId);
                  deliver();
                }
              }
              debug2("connect :: sending queued packets");
              deliver();
            });
            this.on("close", function() {
              debug2("close :: connected set to `false`");
              this.connected = false;
              debug2("close :: clearing connackTimer");
              clearTimeout(this.connackTimer);
              debug2("close :: clearing ping timer");
              ...
              debug2("close :: calling _setupReconnect");
              this._setupReconnect();
            });
            EventEmitter.call(this);
            debug2("MqttClient :: setting up stream");
            this._setupStream();
          }
          ...
          MqttClient.prototype._setupStream = function() {
            ...
            const packets = [];
            debug2("_setupStream :: calling method to clear reconnect");
            this._clearReconnect();
            debug2("_setupStream :: using streamBuilder provided to client to create stream");
            this.stream = this.streamBuilder(this);
            parser.on("packet", function(packet) {
              debug2("parser :: on packet push to packets array.");
              packets.push(packet);
            });
            ...
            function work() {
              debug2("work :: getting next packet in queue");
              const packet = packets.shift();
              if (packet) {
                debug2("work :: packet pulled from queue");
                that._handlePacket(packet, nextTickWork);
              } else {
                debug2("work :: no packets in queue");
                const done = completeParse;
                completeParse = null;
                debug2("work :: done flag is %s", !!done);
                if (done)
                  done();
              }
            }
            writable._write = function(buf, enc, done) {
              completeParse = done;
              debug2("writable stream :: parsing buffer");
              parser.parse(buf);
              work();
            };
            function streamErrorHandler(error) {
              debug2("streamErrorHandler :: error", error.message);
              if (socketErrors.includes(error.code)) {
                debug2("streamErrorHandler :: emitting error");
                that.emit("error", error);
              } else {
                nop(error);
              }
            }
            debug2("_setupStream :: pipe stream to writable stream");
            this.stream.pipe(writable);
            this.stream.on("error", streamErrorHandler);
            this.stream.on("close", function() {
              debug2("(%s)stream :: on close", that.options.clientId);
              flushVolatile(that.outgoing);
              debug2("stream: emit close to MqttClient");
              that.emit("close");
            });
            debug2("_setupStream: sending packet `connect`");
            ...
            this.connackTimer = setTimeout(function() {
              debug2("!!connectTimeout hit!! Calling _cleanUp with force `true`");
              that._cleanUp(true);
          ...
          MqttClient.prototype._handlePacket = function(packet, done) {
            ...
            debug2("_handlePacket :: emitting packetreceive");
          ...
          MqttClient.prototype.publish = function(topic, message, opts, callback) {
            debug2("publish :: message `%s` to topic `%s`", message, topic);
            const options = this.options;
            ...
            const publishProc = function() {
              ...
              switch (opts.qos) {
                ...
                case 2:
                  ...
                  debug2("MqttClient:publish: packet cmd: %s", packet.cmd);
          ...
          MqttClient.prototype.subscribe = function() {
            ...
            if (this._checkDisconnecting(callback)) {
              debug2("subscribe: discconecting true");
              return this;
            }
            const defaultOpts = {
              qos: 0
            ...
            if (Array.isArray(obj)) {
              obj.forEach(function(topic) {
                debug2("subscribe: array topic %s", topic);
                if (!Object.prototype.hasOwnProperty.call(that._resubscribeTopics, topic) || that._resubscribeTopics[topic].qos < opts.qos || resubscribe) {
                  ...
                  debug2("subscribe: pushing topic `%s` and qos `%s` to subs list", currentOpts.topic, currentOpts.qos);
                  subs.push(currentOpts);
                }
              });
            } else {
              Object.keys(obj).forEach(function(k) {
                debug2("subscribe: object topic %s", k);
                if (!Object.prototype.hasOwnProperty.call(that._resubscribeTopics, k) || that._resubscribeTopics[k].qos < obj[k].qos || resubscribe) {
                  ...
                  debug2("subscribe: pushing `%s` to subs list", currentOpts);
                  subs.push(currentOpts);
                }
              });
            }
            ...
            const subscribeProc = function() {
              ...
              if (that.options.resubscribe) {
                debug2("subscribe :: resubscribe true");
                const topics = [];
              ...
              debug2("subscribe :: call _sendPacket");
          ...
          MqttClient.prototype.unsubscribe = function() {
            ...
            const unsubscribeProc = function() {
              ...
              debug2("unsubscribe: call _sendPacket");
              that._sendPacket(packet);
              return true;
            };
          ...
          MqttClient.prototype.end = function(force, opts, cb) {
            const that = this;
            debug2("end :: (%s)", this.options.clientId);
            ...
            function closeStores() {
              debug2("end :: closeStores: closing incoming and outgoing stores");
              that.disconnected = true;
              that.incomingStore.close(function(e1) {
                that.outgoingStore.close(function(e2) {
                  debug2("end :: closeStores: emitting end");
                  that.emit("end");
                  if (cb) {
                    const err = e1 || e2;
                    debug2("end :: closeStores: invoking callback with args");
                    cb(err);
                  }
                });
              });
            ...
            function finish() {
              debug2("end :: (%s) :: finish :: calling _cleanUp with force %s", that.options.clientId, force);
              that._cleanUp(force, () => {
                debug2("end :: finish :: calling process.nextTick on closeStores");
            ...
            if (!force && Object.keys(this.outgoing).length > 0) {
              debug2("end :: (%s) :: calling finish in 10ms once outgoing is empty", that.options.clientId);
              this.once("outgoingEmpty", setTimeout.bind(null, finish, 10));
            } else {
              debug2("end :: (%s) :: immediately calling finish", that.options.clientId);
              finish();
            }
            return this;
          };
          ...
          MqttClient.prototype._reconnect = function() {
            debug2("_reconnect: emitting reconnect to client");
            this.emit("reconnect");
            if (this.connected) {
              this.end(() => {
                this._setupStream();
              });
              debug2("client already connected. disconnecting first.");
            } else {
              debug2("_reconnect: calling _setupStream");
              this._setupStream();
            }
          };
          MqttClient.prototype._setupReconnect = function() {
            ...
            if (!that.disconnecting && !that.reconnectTimer && that.options.reconnectPeriod > 0) {
              if (!this.reconnecting) {
                debug2("_setupReconnect :: emit `offline` state");
                this.emit("offline");
                debug2("_setupReconnect :: set `reconnecting` to `true`");
                this.reconnecting = true;
              }
              debug2("_setupReconnect :: setting reconnectTimer for %d ms", that.options.reconnectPeriod);
              that.reconnectTimer = setInterval(function() {
                debug2("reconnectTimer :: reconnect triggered!");
                that._reconnect();
              }, that.options.reconnectPeriod);
            } else {
              debug2("_setupReconnect :: doing nothing...");
            }
          };
          MqttClient.prototype._clearReconnect = function() {
            debug2("_clearReconnect : clearing reconnect timer");
          ...
          MqttClient.prototype._cleanUp = function(forced, done) {
            ...
            if (done) {
              debug2("_cleanUp :: done callback provided for on stream close");
              this.stream.on("close", done);
            }
            debug2("_cleanUp :: forced? %s", forced);
            if (forced) {
              if (this.options.reconnectPeriod === 0 && this.options.clean) {
                flush(this.outgoing);
              }
              debug2("_cleanUp :: (%s) :: destroying stream", this.options.clientId);
              this.stream.destroy();
            } else {
              const packet = xtend({ cmd: "disconnect" }, opts);
              debug2("_cleanUp :: (%s) :: call _sendPacket with disconnect packet", this.options.clientId);
              this._sendPacket(packet, setImmediate.bind(null, this.stream.end.bind(this.stream)));
            }
            if (!this.disconnecting) {
              debug2("_cleanUp :: client not disconnecting. Clearing and resetting reconnect.");
              this._clearReconnect();
              this._setupReconnect();
            }
            if (this.pingTimer !== null) {
              debug2("_cleanUp :: clearing pingTimer");
              this.pingTimer.clear();
              this.pingTimer = null;
            }
            if (done && !this.connected) {
              debug2("_cleanUp :: (%s) :: removing stream `done` callback `close` listener", this.options.clientId);
              this.stream.removeListener("close", done);
              done();
            }
          };
          MqttClient.prototype._sendPacket = function(packet, cb, cbStorePut) {
            debug2("_sendPacket :: (%s) ::  start", this.options.clientId);
            ...
            if (!this.connected) {
              ...
              debug2("_sendPacket :: client not connected. Storing packet offline.");
            ...
            debug2("_sendPacket :: (%s) ::  end", this.options.clientId);
          };
          MqttClient.prototype._storePacket = function(packet, cb, cbStorePut) {
            debug2("_storePacket :: packet: %o", packet);
            debug2("_storePacket :: cb? %s", !!cb);
            cbStorePut = cbStorePut || nop;
            let storePacket = packet;
            if (storePacket.cmd === "publish") {
              storePacket = clone(packet);
          ...
          MqttClient.prototype._setupPingTimer = function() {
            debug2("_setupPingTimer :: keepalive %d (seconds)", this.options.keepalive);
            const that = this;
          ...
          MqttClient.prototype._checkPing = function() {
            debug2("_checkPing :: checking ping...");
            if (this.pingResp) {
              debug2("_checkPing :: ping response received. Clearing flag and sending `pingreq`");
              this.pingResp = false;
              this._sendPacket({ cmd: "pingreq" });
            } else {
              debug2("_checkPing :: calling _cleanUp with force true");
              this._cleanUp(true);
            }
          };
          ...
          MqttClient.prototype._handlePublish = function(packet, done) {
            debug2("_handlePublish: packet %o", packet);
              ...
              if (typeof alias !== "undefined") {
                if (topic.length === 0) {
                  if (alias > 0 && alias <= 65535) {
                    ...
                    if (gotTopic) {
                      topic = gotTopic;
                      debug2("_handlePublish :: topic complemented by alias. topic: %s - alias: %d", topic, alias);
                    } else {
                      debug2("_handlePublish :: unregistered topic alias. alias: %d", alias);
                      this.emit("error", new Error("Received unregistered Topic Alias"));
                      return;
                    }
                  } else {
                    debug2("_handlePublish :: topic alias out of range. alias: %d", alias);
                    this.emit("error", new Error("Received Topic Alias is out of range"));
                    return;
                  }
                } else {
                  if (this.topicAliasRecv.put(topic, alias)) {
                    debug2("_handlePublish :: registered topic: %s - alias: %d", topic, alias);
                  } else {
                    debug2("_handlePublish :: topic alias out of range. alias: %d", alias);
                    this.emit("error", new Error("Received Topic Alias is out of range"));
                    return;
            ...
            debug2("_handlePublish: qos %d", qos);
            switch (qos) {
              case 2: {
              ...
              case 0:
                ...
                break;
              default:
                debug2("_handlePublish: unknown QoS. Doing nothing.");
                break;
            }
          };
          ...
          MqttClient.prototype._handleAck = function(packet) {
            ...
            let err;
            if (!cb) {
              debug2("_handleAck :: Server sent an ack in error. Ignoring.");
              return;
            }
            debug2("_handleAck :: packet type", type);
            switch (type) {
              case "pubcomp":
              case "puback": {
          ...
          MqttClient.prototype._resubscribe = function() {
            ...
            if (!this._firstConnection && (this.options.clean || this.options.protocolVersion === 5 && !this.connackPacket.sessionPresent) && _resubscribeTopicsKeys.length > 0) {
              if (this.options.resubscribe) {
                if (this.options.protocolVersion === 5) {
                  debug2("_resubscribe: protocolVersion 5");
              ...
              function storeDeliver() {
                ...
                if (!that.disconnecting && !that.reconnectTimer) {
                  ...
                  if (that.messageIdProvider.register(packet2.messageId)) {
                    ...
                    debug2("messageId: %d has already used.", packet2.messageId);
                  }
      ...
      const debug2 = require2("debug")("mqttjs:tcp");
      ...
      const net = require2("net");
      const debug2 = require2("debug")("mqttjs:tls");
      ...
      (function(process) {
        (function() {
          ...
          const debug2 = require2("debug")("mqttjs:ws");
          const duplexify = require2("duplexify");
          const Transform = require2("readable-stream").Transform;
          const WSS_OPTIONS = [
            "rejectUnauthorized",
      ...
      (function(process) {
        (function() {
          ...
          const xtend = require2("xtend");
          const debug2 = require2("debug")("mqttjs");
          const protocols = {};
      ...
      const debug2 = require2("debug")("mqtt-packet:parser");
      class Parser extends EventEmitter {
        ...
        _parseConnect() {
          ...
          debug2("_parseConnect: packet.clientId: %s", packet.clientId);
          if (flags.will) {
            if (packet.protocolVersion === 5) {
            ...
            debug2("_parseConnect: packet.will.topic: %s", packet.will.topic);
            payload = this._parseBuffer();
            if (payload === null)
              return this._emitError(new Error("Cannot parse will payload"));
            packet.will.payload = payload;
            debug2("_parseConnect: packet.will.paylaod: %s", packet.will.payload);
          }
          if (flags.username) {
            ...
            debug2("_parseConnect: packet.username: %s", packet.username);
          }
          if (flags.password) {
        ...
        _parsePublish() {
          ...
          debug2("_parsePublish: payload from buffer list: %o", packet.payload);
        }
        _parseSubscribe() {
        ...
        _parseConfirmation() {
          ...
          if (this.settings.protocolVersion === 5) {
            if (packet.length > 2) {
              ...
              debug2("_parseConfirmation: packet.reasonCode `%d`", packet.reasonCode);
            } else {
              packet.reasonCode = 0;
            }
            if (packet.length > 3) {
        ...
        _parseMessageId() {
          ...
          debug2("_parseMessageId: packet.messageId %d", packet.messageId);
          return true;
        }
      ...
      (function(Buffer2) {
        (function() {
          ...
          const debug2 = require2("debug")("mqtt-packet:writeToStream");
          ...
          function generate(packet, stream, opts) {
            ...
            debug2("generate: packet.cmd: %s", packet.cmd);
            switch (packet.cmd) {
              case "connect":
          ...
          function publish2(packet, stream, opts) {
            debug2("publish: packet: %o", packet);
            const version = opts ? opts.protocolVersion : 4;
            const settings = packet || {};
...
const messageListenerMapObject = {};
const runEvent = (clientID, topic, message, packet) => {
...
const unRegisterEvent$1 = async (clientID, topic, vm = "none") => {
  const listenerMap = messageListenerMapObject[clientID];
  if (listenerMap) {
    const listenerList = listenerMap.get(topic);
    if (listenerList && listenerList.length) {
      const indexArray = [];
      for (let i = listenerList.length - 1; i >= 0; i -= 1) {
        if (listenerList[i].vm === vm)
          indexArray.push(i);
      }
      if (indexArray.length) {
        indexArray.forEach((index) => listenerList.splice(index, 1));
        if (listenerList.length > 0)
          listenerMap.set(topic, listenerList);
        else
          listenerMap.delete(topic);
      }
    }
  }
};
const registerEvent$1 = async (clientID, topic, callback, vm = "none") => {
  var _a;
  let listenerMap = messageListenerMapObject[clientID];
  if (!listenerMap) {
    messageListenerMapObject[clientID] = /* @__PURE__ */ new Map();
    listenerMap = messageListenerMapObject[clientID];
  }
  await unRegisterEvent$1(clientID, topic, vm);
  listenerMap.has(topic) || listenerMap.set(topic, []);
  (_a = listenerMap.get(topic)) == null ? void 0 : _a.push({ callback, vm });
};
const clearEvent$1 = async (clientID) => {
  const listenerMap = messageListenerMapObject[clientID];
  if (listenerMap)
    listenerMap.clear();
};
const useEventHook = () => ({
  runEvent,
  unRegisterEvent: unRegisterEvent$1,
  registerEvent: registerEvent$1,
  clearEvent: clearEvent$1
});
const eventHook$1 = useEventHook();
const clientObject = {};
const publishBufferObject = {};
const subscribeBufferObject = {};
const getPublishBuffer = (clientID) => {
  if (!publishBufferObject[clientID])
    publishBufferObject[clientID] = [];
  return publishBufferObject[clientID];
};
const getSubscribeBuffer = (clientID) => {
  if (!subscribeBufferObject[clientID])
    subscribeBufferObject[clientID] = [];
  return subscribeBufferObject[clientID];
};
const isConnected$1 = (clientID) => {
  var _a;
  if ((_a = clientObject[clientID]) == null ? void 0 : _a.connected)
    return true;
  return false;
};
const subscribe$1 = async (clientID, topicArray, qos = 1, opts = {
  qos: 1,
  nl: false,
  rap: false,
  rh: 0,
  ...
  var _a, _b;
  opts.qos = qos;
  if (clientObject[clientID] == null || !((_a = clientObject[clientID]) == null ? void 0 : _a.connected)) {
    ...
    (_b = clientObject[clientID]) == null ? void 0 : _b.subscribe(topicArray, opts, callback);
  }
};
const unSubscribe$1 = async (clientID, unTopic, opts = {}, callback) => {
  var _a, _b;
  if (callback) {
    (_a = clientObject[clientID]) == null ? void 0 : _a.unsubscribe(unTopic, opts, callback);
  } else {
    (_b = clientObject[clientID]) == null ? void 0 : _b.unsubscribe(unTopic, opts, () => {
      common.debug().log(`unsubscribe: ${unTopic}`);
    });
  }
};
const publish$1 = async (clientID, topic, message, qos = 0, opts = {
  qos: 0,
  retain: false,
  dup: false,
  properties: {}
  ...
  if (clientObject[clientID] == null || !((_a = clientObject[clientID]) == null ? void 0 : _a.connected)) {
    ...
    clientObject[clientID].publish(topic, message, opts, callback);
  }
};
const onConnectFail = async (clientID) => {
  var _a;
  (_a = clientObject[clientID]) == null ? void 0 : _a.on("error", (error) => {
    var _a2;
    common.debug().log("connect fail:", error);
    (_a2 = clientObject[clientID]) == null ? void 0 : _a2.end();
    eventHook$1.runEvent(clientID, "on-connect-fail", String(error));
  });
};
const onMessage = async (clientID) => {
  var _a;
  (_a = clientObject[clientID]) == null ? void 0 : _a.on("message", (topic, message, packet) => {
    if (message)
      eventHook$1.runEvent(clientID, topic, message, packet);
  });
};
const onConnect = async (clientID, url) => {
  var _a;
  (_a = clientObject[clientID]) == null ? void 0 : _a.on("connect", () => {
    common.debug().log("success connect to host:", url);
    const subscribeBuffer = getSubscribeBuffer(clientID);
    subscribeBuffer.forEach(({ topicArray, qos, opts, callback }) => subscribe$1(clientID, topicArray, qos, opts, callback));
    subscribeBuffer.length = 0;
    const publishBuffer = getPublishBuffer(clientID);
    publishBuffer.forEach(({ topic, message, qos, opts, callback }) => publish$1(clientID, topic, message, qos, opts, callback));
    publishBuffer.length = 0;
    eventHook$1.runEvent(clientID, "on-connect", "");
  });
};
const onReconnect = async (clientID) => {
  var _a;
  (_a = clientObject[clientID]) == null ? void 0 : _a.on("reconnect", () => {
    var _a2, _b;
    common.debug().log("try to reconnect:", (_b = (_a2 = clientObject[clientID]) == null ? void 0 : _a2.options) == null ? void 0 : _b.hostname);
    eventHook$1.runEvent(clientID, "on-reconnect", "");
  });
};
const onEnd = async (clientID) => {
  var _a;
  (_a = clientObject[clientID]) == null ? void 0 : _a.on("end", () => {
    common.debug().log("disconnected");
    eventHook$1.runEvent(clientID, "on-disconnect", "");
  });
};
const disconnect$1 = async (clientID) => {
...
const connect$1 = async (clientID, url, _options) => {
  common.debug().log("mqtt connecting");
  if (clientObject[clientID])
    await disconnect$1(clientID);
...
const reconnect$1 = async (clientID, url, _options) => {
  disconnect$1(clientID);
  connect$1(clientID, url, _options);
  common.debug().log("mqtt reconnecting");
};
const showAllClient$1 = async () => {
  return Object.keys(clientObject);
};
const mqttBaseHook$1 = () => ({
  ...
  publish: publish$1,
  showAllClient: showAllClient$1
});
const useMqttBaseHook = () => mqttBaseHook$1();
...
const subscribe = async (topicArray, qos, opts, callback, clientID) => {
  if (!qos)
    qos = 1;
  if (!opts)
    opts = {
...
const unSubscribe = async (unTopic, opts, callback, clientID) => {
  if (!opts)
    opts = {};
  if (!callback)
    callback = () => {
...
const publish = async (topic, message, qos, opts, callback, clientID) => {
  if (!qos)
    qos = 0;
  if (!opts)
    opts = {
...
const connect = async (url, _options, clientID) => {
  if (!clientID)
    clientID = "default";
  return mqttBaseHook.connect(clientID, url, _options);
};
const reconnect = async (url, _options, clientID) => {
  if (!clientID)
    clientID = "default";
...
const unRegisterEvent = async (topic, vm = "none", clientID) => {
  if (!clientID)
    clientID = "default";
  eventHook.unRegisterEvent(clientID, topic, vm);
};
const registerEvent = async (topic, callback, vm = "none", clientID) => {
  if (!clientID)
    clientID = "default";
  eventHook.registerEvent(clientID, topic, callback, vm);
};
const clearEvent = async (clientID) => {
  if (!clientID)
    clientID = "default";
...
const test = async () => {
  ...
  registerEvent("mqtt-vue-hook/test", (topic, message) => {
